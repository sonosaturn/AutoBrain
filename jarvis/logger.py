...
def summarize_ai_response(text):
    ...
    # Cattura riferimenti a file (es: main.py o @project\file.py)
    files_mentioned = re.findall(r'(@?[\w\-\.\\]+\.\w+)', text)
    files_unique = sorted(list(set(files_mentioned)))
    files_str = f"\n\n**Modificati i seguenti file:** {', '.join(files_unique)}" if files_mentioned else ""
    
    summary_prompt = f"""Riassumi questa risposta dell'IA per l'archiviazione:
...
class ConversationLogger:
    def __init__(self, identity="Jarvis"):
    ...
    def log_interazione_completa(self, user_input, assistant_response, summarize=True):
        ...
        self.log_messaggio("assistant" if self.identity == "Jarvis" else "gemini_cli", assistant_response, summarize=summarize)
